...
namespace AskPam.Crm.IntegratedTests.Conversations
{
    [Collection(Consts.TEST_COLLECTION)]
    public class When_Receive_Email
    {
        readonly TestServerFixture _testServerFixture;

        public When_Receive_Email(TestServerFixture testServerFixture)
        {
        ...
        {
            //Test email reception newemail1@ask-pam.com - 2x
            var data = MessageContent.emailJson.Replace("{0}", _testServerFixture.user1OrganizationId.ToString());
            data = data.Replace("{1}", "newemail1");
            var content = new StringContent(data, Encoding.UTF8, "application/json");
            var response = await _testServerFixture.Client.PostAsync("api/postmark/income", content);
            response.StatusCode.ShouldBe(HttpStatusCode.OK);

            content = new StringContent(data, Encoding.UTF8, "application/json");
            response = await _testServerFixture.Client.PostAsync("api/postmark/income", content);
            response.StatusCode.ShouldBe(HttpStatusCode.OK);

            //Test email reception newemail2@ask-pam.com
            data = MessageContent.emailJson.Replace("{0}", _testServerFixture.user1OrganizationId.ToString());
            data = data.Replace("{1}", "newemail2");
            content = new StringContent(data, Encoding.UTF8, "application/json");
            response = await _testServerFixture.Client.PostAsync("api/postmark/income", content);
            response.StatusCode.ShouldBe(HttpStatusCode.OK);

            //Test email present in database

            var requestConversationData = new ConversationListRequestDto { Filter = ConversationFilter.All, MaxResultCount = 10, SkipCount = 0, Search = "New Email" };
            ...
            conversationList.Items.Count.ShouldBe(2);
