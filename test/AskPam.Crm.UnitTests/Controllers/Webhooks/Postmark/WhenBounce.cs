...
{
    public class WhenBounce : BasePostMarkController
    {
        public WhenBounce()
        {
        ...
        [Fact(DisplayName = "WhenBounce_ShouldAddWarningToConversation")]
        public async Task ShouldAddWarningToConversation()
        {
            var postMarkMessageId = Guid.NewGuid();
            var conversation = await Context.Conversations.Include(c => c.Messages).FirstAsync();
            ...
            await _controller.Bounce(JsonConvert.DeserializeObject<BouncedMail>(MessageContent.BounceJson.Replace("{0}", postMarkMessageId.ToString())));

            conversation = await Context.Conversations.Include(c => c.Messages).ThenInclude(m => m.DeliveryStatus).FirstAsync();
            var deliveryStatus = conversation.Messages.Last().DeliveryStatus.First();

            conversation.Messages.First().DeliveryStatus.Count.ShouldBe(1);
            deliveryStatus.Success.ShouldBeFalse();
